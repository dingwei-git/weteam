...
public class RedisUtil {
    ...
    @Value("${spring.redis.redismaxactive}")
    private String  redisMaxActive;
    @Value("${spring.redis.redismaxidle}")
    private String redisMaxidle;
    @Value("${spring.redis.redismaxwait}")
    ...
    public void initialPool() {
        try {
            ...
            config.setMaxTotal(Integer.valueOf(redisMaxActive));
            config.setMaxIdle(Integer.valueOf(redisMaxidle));
            config.setMaxWaitMillis(Integer.valueOf(redisMaxWait));
            ...
            jedisPool = new JedisPool(config,redisIp, Integer.valueOf(redisport), Integer.valueOf(redisTimeOut), redisPassWord);
        } catch (Exception e) {
            /*try{
                //如果第一个IP异常，则访问第二个IP
                JedisPoolConfig config = new JedisPoolConfig();
